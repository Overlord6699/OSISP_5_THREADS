...
LPCSTR input_file = "input.txt";
LPCSTR output_file = "output.txt";
	...
	PSTR lpCmdLine, INT nCmdShow)
{
	...
	BOOL res = ReadFile(hFile, buffer, BUFFER_SIZE - 1, &dwBytesRead, NULL);
	if (res == -1)
	{
	...
	//бесконечное ожидание всех потоков
	WaitForMultipleObjects(THREAD_COUNT, hThreads, TRUE, INFINITE);
	tmp = head;
	int currentLength = 0;
...
{
	HANDLE hFile = CreateFile(output_file, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

	if (hFile == NULL)
	{
		printf("Can't find such file!");
		return;
	}
